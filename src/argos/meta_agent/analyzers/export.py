...
__all__ = ["BaseExportAnalyzer", "JsonExportAnalyzer", "PickleExportAnalyzer"]
...
from iden.io import save_json, save_pickle
...
class BaseExportAnalyzer(BaseAnalyzer):
    ...
    Example:
        ...
        ...     path = pathlib.Path(tmp).joinpath("analysis.json")
        ...     analyzer = JsonExportAnalyzer(analyzer=Analyzer(Analysis("my analysis")), path=path)
        ...     analyzer
...
class PickleExportAnalyzer(BaseExportAnalyzer):
    ...
    Example:
        ...
        >>> import pathlib, tempfile, polars as pl
        >>> from argos.meta_agent.analyses import Analysis
        >>> from argos.meta_agent.analyzers import Analyzer, PickleExportAnalyzer
        >>> with tempfile.TemporaryDirectory() as tmp:
        ...     path = pathlib.Path(tmp).joinpath("analysis.pickle")
        ...     analyzer = PickleExportAnalyzer(
    ...
    def _export(self, analysis: BaseAnalysis) -> None:
        save_pickle(analysis.to_primitive(), self._path, **self._kwargs)
