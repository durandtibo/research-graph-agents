...
def test_json_history_file_does_not_exist(tmp_path: Path) -> None:
    history = JsonHistory(tmp_path.joinpath("data"))
    assert history.path.is_file()
    assert load_json(history.path) == []


def test_json_history_file_already_exists(json_path: Path) -> None:
    history = JsonHistory(json_path)
    assert history.path.is_file()
    assert load_json(history.path) == [{"key": "value"}]


def test_json_history_file_append_empty(tmp_path: Path) -> None:
...
def test_json_history_file_append_not_empty(json_path: Path) -> None:
    history = JsonHistory(json_path)
    history.append({"hello": "world"})
    assert history.path.is_file()
...
def test_json_history_file_get_values_empty(tmp_path: Path) -> None:
    history = JsonHistory(tmp_path.joinpath("data").joinpath("history.json"))
    assert history.get_values() == []


def test_json_history_file_get_values_not_empty(json_path: Path) -> None:
    history = JsonHistory(json_path)
