...
def test_check_rich_with_package() -> None:
    with patch("argos.utils.imports.rich.is_rich_available", lambda: True):
        check_rich()


def test_check_rich_without_package() -> None:
    with (
        patch("argos.utils.imports.rich.is_rich_available", lambda: False),
...
def test_rich_available_without_package() -> None:
    with patch("argos.utils.imports.rich.is_rich_available", lambda: False):
        fn = rich_available(my_function)
        assert fn(2) is None
