...
def test_dataframe_to_csv_empty() -> None:
    assert dataframe_to_csv(pl.DataFrame()) == "_No data available._"
...
###########################################
#     Tests for dataframe_to_markdown     #
###########################################


def test_dataframe_to_markdown_empty() -> None:
    assert dataframe_to_markdown(pl.DataFrame()) == "_No data available._"


def test_dataframe_to_markdown_single_row() -> None:
    frame = pl.DataFrame({"loss": [0.5], "accuracy": [0.9]})
    assert dataframe_to_markdown(frame) == (
        "| loss | accuracy |\n|------|----------|\n| 0.5  | 0.9      |"
    )


def test_dataframe_to_markdown_multiple_rows() -> None:
...
def test_dataframe_to_markdown_single_column() -> None:
    frame = pl.DataFrame({"loss": [0.5, 0.3]})
    assert dataframe_to_markdown(frame) == "| loss |\n|------|\n| 0.5  |\n| 0.3  |"


def test_dataframe_to_markdown_string_values() -> None:
    frame = pl.DataFrame({"model": ["resnet50", "vgg16"]})
    assert dataframe_to_markdown(frame) == (
        "| model    |\n|----------|\n| resnet50 |\n| vgg16    |"
    )


def test_dataframe_to_markdown_wide_column_name() -> None:
    frame = pl.DataFrame({"very_long_column_name": [1]})
    assert dataframe_to_markdown(frame) == (
        "| very_long_column_name |\n|-----------------------|\n| 1                     |"
    )


def test_dataframe_to_markdown_wide_value() -> None:
    frame = pl.DataFrame({"x": ["a_very_long_value"]})
    assert dataframe_to_markdown(frame) == (
        "| x                 |\n|-------------------|\n| a_very_long_value |"
    )
